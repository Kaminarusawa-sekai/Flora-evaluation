...
from schemas.schemas import Stage3BOutput, TestScenario, TestStep
...
class ScenarioAdapter(ModuleAdapter):
    ...
    def _extract_topology(self, input_data: Dict, config: Dict) -> Dict:
        ...
        # 使用默认数据库 'neo4j'，而不是 'flora_topology'
        # Neo4j 默认数据库名称是 'neo4j'
        database = input_data.get('database', 'neo4j')
        
        print(f"[ScenarioAdapter] Connecting to Neo4j database: {database}")
        ...
        try:
            ...
            print(f"[ScenarioAdapter] Extracted {len(apis)} APIs, {len(dependencies)} dependencies, {len(entities)} entities")
            ...
            driver.close()
            print(f"[ScenarioAdapter] Error extracting topology from Neo4j: {e}")
            print(f"[ScenarioAdapter] Make sure the database '{database}' exists and contains API topology data")
            raise
    ...
    def validate_input(self, input_data: Dict) -> bool:
        ...
        is_neo4j_source = input_data.get('source_type') == 'neo4j' and 'database' in input_data
        return has_topology or has_neo4j_config or is_neo4j_source
