...
class EventPublisher:
    ...
    def __init__(
        ...
        self.disabled = os.getenv("COOP_DISABLE_EVENT_BUS", "") == "1"
        if self.disabled:
            # Override async publish methods to avoid un-awaited coroutine warnings.
            self.publish = self._publish_disabled  # type: ignore[assignment]
            self.publish_task_event = self._publish_task_event_disabled  # type: ignore[assignment]
    ...
    def _publish_disabled(self, *args, **kwargs) -> None:
        return None

    def _publish_task_event_disabled(self, *args, **kwargs) -> None:
        return None
    ...
    async def get_signal_status(
        ...
        """
        if self.disabled:
            return {"signal": SignalStatus.NORMAL}
