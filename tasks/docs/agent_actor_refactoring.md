...
### 0 conversation_manager完善
capabilities/context/conversation_manager.py中将接口部分移到对应的文件
在ConversationManager的initialize中初始化IntentRouter
去掉agent_actor相关的逻辑，将逻辑移入ConversationManager



### 0️检查是否是参数补完消息，是参数补完消息直接转发给对应的ExecutionActor
...
def _is_leaf_node(self, task_description: str) -> bool:
    ...
    使用agents/tree/tree_manager.py判断是否是叶子节点
    """
...
def _should_execute_in_parallel(self, subtasks: List[Dict[str, Any]]) -> bool:
    ...
    # 判断标准：
    由llm判断是否该任务可以多参数试验看哪个效果更好，具体的判断放在capbility里，这里引用
