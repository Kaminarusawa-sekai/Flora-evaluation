...
class ResultAggregatorActor(Actor):
    ...
    def _handle_result_aggregator_request(self, msg: ResultAggregatorTaskRequestMessage, sender: Any) -> None:
        ...
        try:
            # 检查是否为 LEAF 类型（强制作为叶子节点执行，避免递归）
            force_leaf = getattr(task_spec, 'type', '').upper() == 'LEAF'
            agent_ref = self._get_or_create_actor_ref(agent_id, force_leaf=force_leaf)
    ...
    def _get_or_create_actor_ref(self, agent_id: str, force_leaf: bool = False):
        if agent_id not in self._actor_ref_cache:
            if not self.registry:
                raise ValueError("Registry not initialized in ResultAggregator")

            # 如果 force_leaf 为 True（来自 LEAF 类型任务），强制创建 LeafActor
            is_leaf = force_leaf or self._is_leaf_node(agent_id)
    ...
    def _handle_task_completed_message(self, msg: TaskCompletedMessage, sender: Any) -> None:
        ...
        if msg.status == "SUCCESS":
            # 传递 agent_id 以便在结果中保留
            self._process_success(msg.task_id, msg.result, getattr(msg, 'agent_id', None))
    ...
    def _process_success(self, task_id: str, result: Any, agent_id: str = None) -> None:
        if not task_id:
            return
        ...
        event_bus.publish_task_event(
            ...
            task_path=self._base_task_path,
            source="ResultAggregatorActor",
            agent_id=agent_id or "result_aggregator",
            user_id=self.current_user_id,
            data={
                "sub_task_id": task_id,
                "result": result,
    ...
    def _check_completion(self) -> None:
        ...
        if not self._pending_tasks:
            ...
            if self._failed_tasks:
                ...
                # 单个成功 - 提取结果和agent_id
                task_data = next(iter(self._completed_tasks.values()))
                # 兼容新旧格式
                ...
                self.send(self._creator, TaskCompletedMessage(
                    ...
                    task_path=self._base_task_path,
                    message_type=MessageType.TASK_COMPLETED,
                    result=self._completed_tasks,  # 已包含agent_id信息
                    status="SUCCESS",
                    agent_id=None
                ))
