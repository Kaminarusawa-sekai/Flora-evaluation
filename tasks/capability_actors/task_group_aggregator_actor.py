...
class TaskGroupAggregatorActor(Actor):
        ...
        try:
            ...
            # 2. 处理标准对象类型的完成消息 (来自 ResultAggregator 或 MCP)
                ...
                if msg.status in ["SUCCESS"]:
                    # 传递完整的msg以保留agent_id信息
                    self._handle_step_success(msg, sender)
    ...
    def _execute_next_step(self) -> None:
        ...
        if current_task.is_parallel:
            ...
            if task_type == "AGENT":
                ...
                # LEAF -> 直接作为叶子节点执行，避免递归
                # ResultAggregator 会根据 agent_id 判断是否为叶子节点
                logger.info(f"--> Route: LEAF execution for Step {current_task.step} (forced leaf)")
                self._dispatch_to_result_aggregator(current_task)
    ...
    def _handle_step_success(self, msg: TaskCompletedMessage, sender: Actor) -> None:
        """通用步骤成功回调"""
        current_task = self._get_current_step()
        step = current_task.step
        logger.info(f"Step {step} succeeded.")

        # 从msg中提取result和agent_id
        result = msg.result
        ...
        event_bus.publish_task_event(
            ...
            task_path=self._task_path,
            source="TaskGroupAggregatorActor",
            agent_id=agent_id or "task_group_aggregator",
            user_id=self.current_user_id,
            data={
                "step": step,
                "step_description": current_task.description,
        ...
        # 1. 存储结果 (Specific Key) - 包含agent_id
        ...
        if isinstance(result, dict):
            for key, value in result.items():
                ...
                if isinstance(value, ContextEntry):
                    ...
                    self.enriched_context[key] = ContextEntry(
                        value=value,
                        source=source,
                        task_path=task_path,
                        timestamp=time.time(),
